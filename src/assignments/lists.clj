...
(defn map'
  ...
  ([f & colls]
   (if (= (count colls) 1)
     ...
     (loop [colls (first colls)
            result-colls []]
       (if (empty? colls)
         result-colls
         (recur (rest colls)
                (conj result-colls (f (first colls))))))

     (loop [colls colls
            result-colls []]
       (if (every? empty? colls)
         result-colls
         (recur (map' rest colls)
                (conj result-colls (apply f (map' first colls)))))))))
...
(defn distinct'
  ...
  (letfn [(get-distinct-element [r c]
    ...
    (lazy-seq (get-distinct-element [] coll))))
